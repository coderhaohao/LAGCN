...
class Sampler():
    ...
    def vr_sample(self, node_list, level, ch_info, is_distinct=False, is_filt_self=False):
        step_list = []
        step_list.append([[_] for _ in node_list])
        for i in range(level):
            last_list = step_list[-1]
        ...
        samp_list = [[[_] for _ in step_list[0]]]

        for i in range(level):
            nei_list = step_list[i + 1]
            s_size, s_time = ch_info[i]

            single_samp = [
                [random.sample(nei, s_size) if len(nei) > s_size else nei for j in range(s_time)]
                for nei in nei_list]
            samp_list.append(single_samp)

        total_list = list(set(chain(*[list(chain(*list(chain(*samp)))) for samp in samp_list])))
        node_map = dict(zip(total_list, range(len(total_list))))

        idx_list = []
        for step_ind, step in enumerate(samp_list):
            row_idx = list(chain(*[[i for _ in nei] for i, nei in enumerate(list(chain(*step)))]))
            col_idx = list(chain(*[[node_map[_] for _ in nei] for nei in list(chain(*step))]))
            idx_list.append((row_idx, col_idx))
        return total_list, idx_list, len(node_list)

    def vrnw_sample(self, node_list, level, ch_info, step2_num, is_distinct=False, is_filt_self=False):
        nei_list = [list(self.neigh_map[_]) for _ in node_list]
        #         step_list.append([[_] for _ in node_list])
        samp_list = [[[[_] for _ in node_list]]]
        if level >= 1:
            s_size = ch_info[0][0]
            s_time = ch_info[0][1]
            samp_list_1 = [
                [random.sample(list(self.neigh_map[_]), s_size)
                 if len(self.neigh_map[_]) > s_size else list(self.neigh_map[_])
                 for j in range(s_time)]
                for _ in node_list]
            samp_list.append(samp_list_1)
        ...
        if level >= 2:
            ...
            for node in node_list:
                ...
                for i in range(s_time):
                    step1_nei = list(self.neigh_map[node])
                    if len(step1_nei) > step2_num:
                        step1_nei = random.sample(step1_nei, step2_num)
                    step2_nei = list(chain(*[list(self.neigh_map[_]) for _ in step1_nei]))
                    if len(step2_nei) > s_size:
                        step2_nei = random.sample(step2_nei, s_size)
                    node_sample.append(step2_nei)
                samp_list_2.append(node_sample)

            samp_list.append(samp_list_2)
        # print(samp_list)
        total_list = list(set(chain(*[list(chain(*list(chain(*samp)))) for samp in samp_list])))
        node_map = dict(zip(total_list, range(len(total_list))))
