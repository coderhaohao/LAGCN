...
flags.DEFINE_string('save_file', '', 'Store the Best')
...
flags.DEFINE_string('ch_dir', '', 'Change the work dir')
os.environ["CUDA_VISIBLE_DEVICES"] = str(FLAGS.gpu)

if len(FLAGS.ch_dir)>0:
    os.chdir(FLAGS.ch_dir)
# Set random seed
seed = FLAGS.seed
np.random.seed(seed)
...
def main(rank1, rank0):
    ...
    if len(FLAGS.save_file)>0:
        with open(FLAGS.save_file,'a') as f:
            f.write('ASGCN %.4f'%max_acc)
            f.write('\n')
    # Testing
