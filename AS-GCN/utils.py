...
from vrn_sampler import Sampler
import random
...
def load_data(dataset_str):
    ...
    objects = []
    for i in range(len(names)):
        print(path+"data/ind.{}.{}".format(dataset_str, names[i]))
...
def loadRedditFromNPZ(dataset_dir):
    path = '../LAGCN/'
    adj = sp.load_npz(path+dataset_dir+"_adj.npz")
    data = np.load(path+dataset_dir+".npz")
...
def prepare_pubmed(dataset, max_degree):
    ...
    # print('attack ready')
    # attack_num = 0
    # print('attack num %d'%(attack_num))
    # tr_idx = np.where(train_mask)[0]
    # va_idx = np.where(val_mask)[0]
    # ts_idx = np.where(test_mask)[0]
    # idx_list = np.concatenate([tr_idx], axis=0)
